#!/usr/bin/python
"""Provide OPM fluidics-controller functions.
...
def lookup_valve(source_name):
    """Convert an ASU well name to an MVP valve position.

    Parameters
    ----------
    source_name : str
        ASU controller source or well name.

    Returns
    -------
    list[int] or None
        MVP unit and port numbers, or ``None`` for an unknown source.
    """
    valve_dict = {
        "B01": [0, 1],
        "B02": [0, 2],
...
def run_fluidic_program(r_idx, df_program, mvp_controller, pump_controller):
    """Run one round from an ASU fluidics program.

    Parameters
    ----------
    r_idx : int
        Zero-based fluidics round index.
    df_program : pandas.DataFrame
        Complete program generated by the ASU fluidics program generator.
    mvp_controller : HamiltonMVP
        Initialized Hamilton MVP valve-chain controller.
    pump_controller : APump
        Initialized pump controller.

    Returns
    -------
    bool
        ``True`` after all steps in the selected round complete.
    """
