'''
QtWidget for setting the OPM v2 configuration.
...
class OPMSettingsV2(QWidget):
    ...
    def __init__(self,
        ...
        config['acq_config']['DAQ']['channel_powers'] = [0] * n_channels
        config['acq_config']['DAQ']['channel_exposures_ms'] = [0] * n_channels
        
        # force stage scan range to max
        config['acq_config']['stage_scan']['max_stage_scan_range_um'] = 1000
        
        # force the scan step size to be 0.4
        config['acq_config']['DAQ']['scan_axis_step_um'] = 0.4
    ...
    def _update_channel_state(
        ...
        self.config['acq_config']['DAQ']['channel_exposures_ms'][channel_index] = exposure_ms
...
OPMSettings = OPMSettingsV2


if __name__ ==  '__main__':
    app = QApplication(sys.argv)
    config_path = Path(Path(Path(sys.path[0]).parent).parent) / Path('opm_config_v2.json')
