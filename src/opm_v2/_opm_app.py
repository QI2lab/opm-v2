...
import numpy as np
from pymmcore_gui import WidgetAction, __version__, create_mmgui
...
def main() -> None:
    ...
    # Load mmcore instance
    mmc = win.mmcore

    # Modify the MDA widget to set default save directory and grid plan mode
    ...
    # Optionally enable debug logging for the MMCore instance
    if DEBUGGING:
    ...
    opmNIDAQ = OPMNIDAQ(
        ...
        projection_mirror_neutral_v=float(
            str(config["NIDAQ"]["projection_mirror_neutral_v"])
        ),
    ...
    FLUIDICS_ENABLED = bool(config["OPM"]["fluidics_enabled"])
    if FLUIDICS_ENABLED:
    ...
    def update_live_state(device_name=None, property_name=None):
        ...
        if DEBUGGING:
            ...
            opmNIDAQ_update_state.projection_mirror_calibration = (
                _projection_calibration
            )
        ...
        # Grab gui values
        _exposure_ms = round(
            float(mmc.getProperty(config["Camera"]["camera_id"], "Exposure")), 0
        )
        _image_mirror_range_um = np.round(
        ...
        if not (crop_y == mmc.getROI()[-1]):
            ...
            mmc.setROI(
                config["Camera"]["roi_center_x"]
                - int(config["Camera"]["roi_crop_x"] // 2),
        ...
        if mmc.getExposure() != _exposure_ms:
            mmc.setProperty(
                str(config["Camera"]["camera_id"]), "Exposure", _exposure_ms
            )
            mmc.waitForDevice(str(config["Camera"]["camera_id"]))
    ...
    opmSettings_widget.widgets["AO"]["mirror_state"].currentIndexChanged.connect(
    ...
    def custom_execute_mda(output: Path | str | None) -> None:
        ...
        if not OPM_ACQUISITION:
            ...
            if optimize_now:
                opm_events, handler = setup_optimizenow(mmc=mmc, config=config)
            else:
                if output:
