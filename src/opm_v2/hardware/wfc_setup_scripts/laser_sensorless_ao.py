...
spherical_modes_first_plus_defocus = [
    7,
    14,
    23,
    2,
...
mm_configPath = Path(
    r"C:\Users\qi2lab\Documents\github\opm_v2\OPM_alignment_config.cfg"
)
...
output_root_path = Path(
    r"C:\Users\qi2lab\Documents\github\opm_v2\src\opm_v2\hardware\wfc_setup_scripts"
)
...
def get_metric(image, metric_to_use):
    """Compute the selected adaptive-optics image metric.

    Parameters
    ----------
    image : numpy.ndarray
        Image from which to compute the metric.
    metric_to_use : str
        Metric selector such as ``brightness`` or ``DCT``.

    Returns
    -------
    float
        Computed metric, or NaN when the selector is unknown.
    """
...
def acquire_zstack(image_type="max"):
    """Acquire and reduce a stack along the O3 focus axis.

    Parameters
    ----------
    image_type : str
        Stack reduction, either ``max`` or ``sum``.

    Returns
    -------
    numpy.ndarray
        Reduced z-stack image.
    """
...
def metric_from_fit(a, b, c, delta):
    """Evaluate a quadratic metric fit at a modal displacement.
    ...
    delta : float
        Modal displacement at which to evaluate the fit.
...
def round_to_sigfigs(x):
    """Round a number to the configured significant-figure count.
...
def optimize_modes(
    ...
    metric_to_use : str
        Image metric used to score each perturbation.
    num_iterations : int
        Number of optimization passes.
    num_mode_steps : int
        Perturbations sampled for each mirror mode.
    starting_coef_delta : float
        Initial modal perturbation magnitude.
    coef_delta_scale : float
        Per-iteration reduction applied to the perturbation magnitude.
    modes_to_optimize : list[int]
        Ordered mirror-mode indices to optimize.
    save_dir_path : Path or None
        Directory in which to save optimization results.
    verbose : bool
        Whether to print optimization progress.
    display_data : bool
        Whether to update interactive image figures.
    image_crop : int or None
        Optional square crop width in pixels.
    """
    # ---------------------------------------------#
    ...
    mode_deltas = [
        (starting_coef_delta - k * starting_coef_delta * (1 - coef_delta_scale))
    ...
    if verbose:
        print(f"+++++ Starting A.O. optimization ++++++\n   Metric: {metric_to_use}")

    try:
        starting_image = acquire_zstack()
        if image_crop:
            starting_image = ao.get_cropped_image(
                starting_image, crop_size=image_crop, center=pixel_center
            )
    ...
    if display_data:
        ...
        def update_image2(new_data):
            """Refresh the candidate-image display.

            Parameters
            ----------
            new_data : numpy.ndarray
                Candidate camera image to display.
            """
        ...
        def update_image1(new_data, metric):
            """Refresh the accepted-image display and metric.

            Parameters
            ----------
            new_data : numpy.ndarray
                Accepted camera image to display.
            metric : float
                Metric associated with the accepted image.
            """
            new_data = new_data / np.max(new_data)
    ...
    for k in range(num_iterations):
        ...
        for mode in modes_to_optimize:
            ...
            for delta in deltas:
                ...
                if not (success):
                    ...
                    if image_crop:
                        image = ao.get_cropped_image(
                            image, crop_size=image_crop, center=pixel_center
                        )
                ...
                if verbose:
                    print(
                        f"    + Delta={delta:.4f}, Metric = {metric:.6f}, Success = {success} +"
                    )
            ...
            if optimal_delta != 0:
                ...
                if image_crop:
                    optimal_image = ao.get_cropped_image(
                        optimal_image, crop_size=image_crop, center=pixel_center
                    )
                optimal_metric = round_to_sigfigs(
                    get_metric(optimal_image, metric_to_use)
                )
                ...
                if round_to_sigfigs(optimal_metric) >= round_to_sigfigs(
                    current_optimal_metrics[-1] * METRIC_PERC_THRESHOLD
                ):
