"""Run the WaveKit wavefront-sensor closed-loop correction example."""
...
wavefront_corrector = wkpy.WavefrontCorrector(
...
corr_data_manager = wkpy.CorrDataManager(
    ...
    interaction_matrix_file_path=str(wfc_correction_file_path),
)
"""Connect hardware
"""
...
if output_file_path.exists():
    best_positions = np.asarray(
        wavefront_corrector.get_positions_from_file(str(output_file_path)),
        dtype=np.float32,
    )
    print(
        f"Initial wavefront corrector positions: {wavefront_corrector.get_current_positions()}"
    )
    wavefront_corrector.move_to_absolute_positions(best_positions)
    print(
        f"Starting wavefront corrector positions: {wavefront_corrector.get_current_positions()}"
    )
else:
...
wkpy.SlopesPostProcessor.apply_filter(
    ref_hasoslopes, False, False, False, False, False, False
)
...
try:
    while True:
        ...
        delta_hasoslopes = wkpy.SlopesPostProcessor.apply_substractor(
            hasoslopes, ref_hasoslopes
        )
        phase = wkpy.Phase(
        ...
        if best_value == None:
            print(
                "Starting RMS value : "
                + str(np.round(phase.get_statistics().rms, 5))
                + "\t(Press any key or Ctrl+C to stop the closed loop)"
            )
        ...
        wavefront_corrector.move_to_relative_positions(delta_commands)
        if msvcrt.kbhit():
            break
...
wavefront_corrector.move_to_absolute_positions(
    np.asarray(best_commands, dtype=np.float32)
)
