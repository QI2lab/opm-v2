...
wfc = wkpy.WavefrontCorrector(config_file_path=str(wfc_config_file_path))
...
haso_config, haso_specs, _ = wkpy.HasoConfig.get_config(
...
center, radius = wkpy.ComputePupil.fit_zernike_pupil(
    pupil, wkpy.E_PUPIL_DETECTION.AUTOMATIC, wkpy.E_PUPIL_COVERING.CIRCUMSCRIBED, False
)
...
modal_coef.set_data(
    coef_array=np.zeros(n_modes), index_array=np.arange(1, 32 + 1, 1), pupil=pupil
)
# ---------------------------------------------#
...
modal_coef.set_data(
    coef_array=test_coefs, index_array=np.arange(1, 32 + 1, 1), pupil=pupil
)

# create a new haso_slope from the new modal coefficients
haso_slopes = wkpy.HasoSlopes(
    modalcoef=modal_coef, config_file_path=str(haso_config_file_path)
)

deltas = corr_data_manager.compute_delta_command_from_delta_slopes(
