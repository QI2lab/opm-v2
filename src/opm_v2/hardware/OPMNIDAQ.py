...
if daqmx is not None:
    warnings.filterwarnings(
        "ignore", category=daqmx.DAQmxFunctions.StoppedBeforeDoneWarning
    )
_instance_daq = None
...
class OPMNIDAQ:
    ...
    scan_type = "stage"
        This mode is similar to "2d". The camera hardware captures and delivers
        the frames after receiving the configured stage-sync pulse. ASI controls
        only the stage/PLC hardware that produces that pulse.
    ...
    def instance(cls) -> "OPMNIDAQ":
        """Return the process-wide DAQ instance.
    ...
    def __init__(
        ...
        image_mirror_calibration: float = 0.0433,
        projection_mirror_calibration: float = 0.0052,
        ...
        """Initialize the OPM waveform controller.

        Parameters
        ----------
        name : str
            NI-DAQmx device name.
        scan_type : str
            Acquisition waveform mode.
        exposure_ms : float
            Camera exposure in milliseconds.
        laser_blanking : bool
            Whether laser output follows the rolling-shutter open interval.
        image_mirror_calibration : float
            Image-mirror calibration in volts per micrometer.
        projection_mirror_calibration : float
            Projection-mirror calibration in volts per micrometer.
        image_mirror_neutral_v : float
            Image-mirror neutral voltage.
        projection_mirror_neutral_v : float
            Projection-mirror neutral voltage.
        image_mirror_step_um : float
            Image-mirror scan step in micrometers.
        verbose : bool
            Whether to print DAQ status messages.
        simulate : bool
            Whether to use the in-memory DAQ backend.

        Raises
        ------
        RuntimeError
            If NI-DAQmx is unavailable for a physical controller.
        """
        ...
        if not self.simulate and daqmx is None:
            raise RuntimeError(
                "PyDAQmx could not load the NI-DAQmx runtime. Install NI-DAQmx or "
                "construct OPMNIDAQ with simulate=True."
            )
    ...
    def exposure_ms(self) -> float:
        ...
        """
        return getattr(self, "_exposure_s", None) * 1000.0
    ...
    def image_mirror_calibration(self, value: float):
        ...
        """
        if (
            not hasattr(self, "_image_mirror_calibration")
            or self._image_mirror_calibration is None
        ):
    ...
    def projection_mirror_calibration(self, value: float):
        ...
        """
        if (
            not hasattr(self, "_projection_mirror_calibration")
    ...
    def image_mirror_step_um(self, value: float):
        ...
        """
        if (
            not hasattr(self, "_image_mirror_step_um")
            or self._image_mirror_step_um is None
        ):
            self._image_mirror_step_um = value
        else:
            self._image_mirror_step_um = value

        self._image_axis_step_volts = (
            self._image_mirror_step_um * self._image_mirror_calibration
        )
    ...
    def image_mirror_range_um(self, value: float):
        ...
        """
        if (
            not hasattr(self, "_image_mirror_range_um")
            or self._image_mirror_range_um is None
        ):
        ...
        self._image_mirror_min_volt = (
            -(self._image_mirror_range_um * self._image_mirror_calibration) / 2.0
        ...
        self._projection_scan_range_volts = (
            self._image_mirror_range_um * self._projection_mirror_calibration
        )
    ...
    def channel_states(self, value: Sequence):
        ...
        self._active_channels_indices = [
            ind for ind, st in zip(self._do_ind, self._channel_states) if st
        ]
        self._n_active_channels = len(self._active_channels_indices)

    def running(self) -> bool:
        """Report whether waveform playback is active.
    ...
    def set_acquisition_params(
        ...
        """Configure the DAQ for an acquisition.
    ...
    def reset_ao_channels(self):
        """Stop tasks and reset analog outputs to neutral mirror positions."""
        self.clear_tasks()

        if self.simulate:
            self._ao_waveform = np.asarray([self._ao_neutral_positions], dtype=float)
            return

        _ao_waveform = np.column_stack((
            np.full(2, self._ao_neutral_positions[0]),
    ...
    def reset_do_channels(self):
        """Stop tasks, remove handlers, and reset digital outputs to zero."""
        self.clear_tasks()

        if self.simulate:
            self._do_waveform = np.zeros((1, self._num_do_channels), dtype=np.uint8)
            return
    ...
    def generate_waveforms(self):
        ...
        if self.scan_type == "2d":
            ...
            _do_waveform = np.zeros(
                (self.samples_per_do_ch, self._num_do_channels), dtype=np.uint8
            )
            ...
            proj_mirror_sweep_volts = np.linspace(
                self.proj_mirror_min_volt,
                self.proj_mirror_max_volt,
                n_voltage_steps - 1,
            )
            ...
            image_mirror_max_volts = (
                self._image_mirror_min_volt + self._image_axis_range_volts
            )
            image_mirror_sweep_volts = np.linspace(
                self._image_mirror_min_volt, image_mirror_max_volts, n_voltage_steps - 1
            )
            ...
            for ii, ind in enumerate(self._active_channels_indices):
                if self._laser_blanking:
                    _do_waveform[
                        2 * ii :: 2 * int(self._n_active_channels), int(ind)
                    ] = 1
                else:
                    _do_waveform[:, int(ind)] = 1
    ...
    def program_daq_waveforms(self):
        ...
        try:
            ...
            if self._task_di is None:
                ...
                self._task_di.CfgDigEdgeStartTrig(
                    self._channel_di_trigger_from_camera, daqmx.DAQmx_Val_Rising
                )

                # Export DI signal to unused PFI pins, for clock and start
                self._task_di.ExportSignal(
                    daqmx.DAQmx_Val_ChangeDetectionEvent,
                    self._channel_di_change_trigger,
                )
                self._task_di.ExportSignal(
                    daqmx.DAQmx_Val_StartTrigger, self._channel_di_start_trigger
                )
