...
class OPMEngine(MDAEngine):
    def __init__(
        self, mmc, config_path: Path, use_hardware_sequencing: bool = True
    ) -> None:
    ...
    def set_camera_roi(self, camera_roi: list[int, int, int, int]):
        """Set the camera ROI
    ...
    def set_laser_powers(
        self,
        channel_powers: list[bool, bool, bool, bool],
        channel_states: list[bool, bool, bool, bool],
    ):
        """Set the laser powers

        Parameters
        ----------
        channel_powers : list[bool,bool,bool,bool]
            Channel powers list, order to match config["Lasers"]["laser_names"]
        channel_states : list[bool,bool,bool,bool]
            Channel states list, false will be set to 0.0
        """
        laser_id = self._config["Lasers"]["name"]
        laser_names = self._config["Lasers"]["laser_names"]

        for ch_idx, laser_name in enumerate(laser_names):
            property_str = laser_name + " - PowerSetpoint (%)"
            if channel_states[ch_idx]:
                self._mmc.setProperty(laser_id, property_str, channel_powers[ch_idx])
            else:
                self._mmc.setProperty(laser_id, property_str, 0.0)
    ...
    def setup_event(self, event: MDAEvent) -> None:
        ...
        if isinstance(event.action, CustomAction):
            ...
            if action_name == "O2O3-autofocus":
                ...
                self.set_camera_exposure(data_dict["Camera"]["exposure_ms"])
                self.set_camera_roi(data_dict["Camera"]["camera_roi"])
                ...
                while not (np.isclose(current_x, target_x, atol=1.0)) or not (
                    np.isclose(current_y, target_y, atol=1.0)
                ):
                    sleep(0.5)
                    current_x, current_y = self._mmc.getXYPosition()
                ...
                self._mmc.setProperty(
                    self._config["Camera"]["camera_id"], "Trigger", "START"
                )
                self._mmc.waitForDevice(str(self._config["Camera"]["camera_id"]))
                while not (
                    self._mmc.getProperty(
                        self._config["Camera"]["camera_id"], "Trigger"
                    )
                    == "START"
                ):
                    sleep(0.1)
                ...
                self._mmc.setProperty(
                    self._config["Camera"]["camera_id"], "TriggerPolarity", "POSITIVE"
                )
                self._mmc.waitForDevice(str(self._config["Camera"]["camera_id"]))
                while not (
                    self._mmc.getProperty(
                        self._config["Camera"]["camera_id"], "TriggerPolarity"
                    )
                    == "POSITIVE"
                ):
                    sleep(0.1)
                    self._mmc.setProperty(
                        self._config["Camera"]["camera_id"],
                        "TriggerPolarity",
                        "POSITIVE",
                    )
                    self._mmc.waitForDevice(str(self._config["Camera"]["camera_id"]))
                ...
                while not (
                    self._mmc.getProperty(
                        self._config["Camera"]["camera_id"], "TRIGGER SOURCE"
                    )
                    == "EXTERNAL"
                ):
                    sleep(0.1)
                ...
                # Set hardware state to run adaptive optics
                    ...
                    # Setup camera properties, re-enforced in run_ao
                    self.set_camera_exposure(data_dict["Camera"]["exposure_ms"])
                    self.set_camera_roi(data_dict["Camera"]["camera_roi"])

                    # Set laser powers
                    self.set_laser_powers(
                        data_dict["AO"]["channel_powers"],
                ...
                # run adaptive optics over a grid of positions.
                    ...
                    # Set laser powers
                    self.set_laser_powers(
                        data_dict["AO"]["ao_dict"]["channel_powers"],
                        data_dict["AO"]["ao_dict"]["channel_states"],
                    )

                    # Set ASI stage speed for normal moves
                    stage_move_speed = self._config["OPM"]["stage_move_speed"]
                    self._mmc.setProperty(
                        self._mmc.getXYStageDevice(),
                        "MotorSpeedX-S(mm/s)",
                ...
                # The exposure is the same for all channels or only 1 channel is active
                for ch_idx, chan_bool in enumerate(data_dict["DAQ"]["channel_states"]):
                    if chan_bool:
                        exposure_ms = data_dict["Camera"]["channel_exposures_ms"][
                            ch_idx
                        ]
                ...
                # Set the laser powers
                self.set_laser_powers(
                    data_dict["DAQ"]["channel_powers"],
                    data_dict["DAQ"]["channel_states"],
                )
    ...
    def post_sequence_started(self, event):
        ...
        if self.execute_stage_scan:
            self._mmc.setProperty(self._mmc.getXYStageDevice(), "ScanState", "Running")
            self.execute_stage_scan = False
                        ...
                        try:
                            ...
                            if DEBUGGING:
                                print(
                                    ...
                                    f"pos: {int(pos_idx)}",
                                    f"mode coeff:{self.AOMirror.current_coeffs.copy()}",
                                    sep="\n",
                                )
                ...
                if coeffs is not None:
                    ...
                    if DEBUGGING:
                        print(
                            ...
                            f"modal coeffs:{self.AOMirror.current_coeffs.copy()}",
                            sep="\n",
                        )
                else:
    ...
    def teardown_sequence(self, sequence: MDASequence) -> None:
        ...
        self._mmc.waitForDevice(str(self._config["Camera"]["camera_id"]))
        self._mmc.setProperty(
            self._config["Camera"]["camera_id"], "TRIGGER SOURCE", "INTERNAL"
        )
        self._mmc.waitForDevice(str(self._config["Camera"]["camera_id"]))

        stage_move_speed = self._config["OPM"]["stage_move_speed"]
        self._mmc.setProperty(
            self._mmc.getXYStageDevice(), "MotorSpeedX-S(mm/s)", stage_move_speed
        )
        self._mmc.setProperty(
            self._mmc.getXYStageDevice(), "MotorSpeedY-S(mm/s)", stage_move_speed
        )
