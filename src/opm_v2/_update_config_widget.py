...
class OPMSettings(QWidget):
    ...
    def __init__(self, config_path: Path):
        ...
        # Set all channel_states to False
        n_channels = len(config["acq_config"]["DAQ"]["channel_states"])
        config["acq_config"]["DAQ"]["channel_states"] = [False] * n_channels
        config["acq_config"]["DAQ"]["channel_powers"] = [0.0] * n_channels
        config["acq_config"]["DAQ"]["channel_exposures_ms"] = [0.0] * n_channels
        # Set mirror scan range to zero
        config["acq_config"]["DAQ"]["scan_range_um"] = 0
        # Set stage scan range to max
        config["acq_config"]["stage_scan"]["max_stage_scan_range_um"] = 1000
        # Set the camera ROI to defualt
        config["acq_config"]["camera_roi"]["center_x"] = config["Camera"]["center_x"]
        config["acq_config"]["camera_roi"]["center_y"] = config["Camera"]["center_y"]
        config["acq_config"]["camera_roi"]["crop_x"] = config["Camera"]["crop_x"]
        config["acq_config"]["camera_roi"]["crop_y"] = config["Camera"]["crop_y"]
    ...
    def create_spinbox(
        ...
        if isinstance(connect_to_fn, list):
            for fn in connect_to_fn:
                spbx.editingFinished.connect(fn)
        elif connect_to_fn:
            spbx.editingFinished.connect(connect_to_fn)

        return spbx

    def create_combobox(self, items: list, width: int = 80, connect_to_fn=None):
        cmbx = QComboBox()
        cmbx.addItems(items)
    ...
    def create_dbspinbox(
        ...
        if isinstance(connect_to_fn, list):
            for fn in connect_to_fn:
                dbspbx.editingFinished.connect(fn)
        elif connect_to_fn:
            dbspbx.editingFinished.connect(connect_to_fn)
        return dbspbx
    ...
    def create_ui(self):
        ...
        self.spbx_active_channel_power = self.create_dbspinbox(
            value=self.config["acq_config"]["AO"]["channel_power"],
        ...
        self.layout_ao_channel = QHBoxLayout()
        self.layout_ao_channel.addWidget(QLabel("Channel power / exposure:"))
        self.layout_ao_channel.addStretch()
        self.layout_ao_channel.addWidget(self.spbx_active_channel_power)
        self.layout_ao_channel.addWidget(QLabel("%"))
        self.layout_ao_channel.addWidget(self.spbx_ao_exposure)
        self.layout_ao_channel.addWidget(QLabel("ms"))
        ...
        self.layout_ao_mirror_range.addWidget(
            QLabel("Projection/Image mirror range (\u00b5m):")
        )
        ...
        self.layout_num_iters_samples = QHBoxLayout()
        self.layout_num_iters_samples.addWidget(
            QLabel("Number of iterations / samples:")
        )
        self.layout_num_iters_samples.addStretch()
        self.layout_num_iters_samples.addWidget(self.spbx_num_iterations)
        self.layout_num_iters_samples.addWidget(self.spbx_num_mode_samples)
        ...
        self.layout_mode_delta.addWidget(QLabel("Mode Pertubations:"))
        self.layout_mode_delta.addStretch()
        self.layout_mode_delta.addWidget(QLabel("\u00B1\u0394 :"))
        self.layout_mode_delta.addWidget(self.spbx_mode_delta)
        self.layout_mode_delta.addWidget(QLabel("\u03B3 :"))
        self.layout_mode_delta.addWidget(self.spbx_mode_alpha)
        ...
        self.spbx_averaged_frames = self.create_spinbox(
            value=1, min=1, max=100, connect_to_fn=self.update_config
        )
        ...
        self.cmbx_ao_camera_mode = self.create_combobox(
            ["on", "off"], connect_to_fn=self.update_config
        )
        ...
        self.layout_ao_main.addLayout(self.layout_ao_channel)
        self.layout_ao_main.addLayout(self.layout_num_iters_samples)
        ...
        self.widgets.update(
            {
                "AO": {
                    ...
                    "channel_id": self.cmbx_ao_active_channel,
                    "channel_power": self.spbx_active_channel_power,
        ...
        self.cmbx_laser_blanking = self.create_combobox(
            items=["on", "off"], connect_to_fn=self.update_config
        )
        ...
        self.sldr_405_power = self.create_slider(connect_to_fn=self.update_405_spbx)
        self.spbx_405_power = self.create_dbspinbox(
            precision=2, connect_to_fn=[self.update_405_slider, self.update_405_state]
        )
        self.spbx_405_exp = self.create_dbspinbox(
            max=1000, precision=2, connect_to_fn=self.update_405_state
        )
        ...
        self.sldr_488_power = self.create_slider(connect_to_fn=self.update_488_spbx)
        self.spbx_488_power = self.create_dbspinbox(
            precision=2, connect_to_fn=[self.update_488_slider, self.update_488_state]
        )
        self.spbx_488_exp = self.create_dbspinbox(
            precision=2, max=1000, connect_to_fn=self.update_488_state
        )
        ...
        self.sldr_561_power = self.create_slider(connect_to_fn=self.update_561_spbx)
        self.spbx_561_power = self.create_dbspinbox(
            precision=2, connect_to_fn=[self.update_561_slider, self.update_561_state]
        )
        self.spbx_561_exp = self.create_dbspinbox(
            precision=2, max=1000, connect_to_fn=self.update_561_state
        )
        ...
        self.sldr_638_power = self.create_slider(connect_to_fn=self.update_638_spbx)
        self.spbx_638_power = self.create_dbspinbox(
            precision=2, connect_to_fn=[self.update_638_slider, self.update_638_state]
        )
        self.spbx_638_exp = self.create_dbspinbox(
            precision=2, max=1000, connect_to_fn=self.update_638_state
        )
        ...
        self.sldr_705_power = self.create_slider(connect_to_fn=self.update_705_spbx)
        self.spbx_705_power = self.create_dbspinbox(
            precision=2, connect_to_fn=[self.update_705_slider, self.update_705_state]
        )
        self.spbx_705_exp = self.create_dbspinbox(
            precision=2, max=1000, connect_to_fn=self.update_705_state
        )
        ...
        self.spbx_image_scan_range = self.create_dbspinbox(
            value=0,
            max=250,
            precision=2,
            interval=0.1,
        ...
        self.layout_image_scan_range = QHBoxLayout()
        self.layout_image_scan_range.addWidget(QLabel("Image scan range (\u00b5m):"))
        self.layout_image_scan_range.addWidget(self.spbx_image_scan_range)
        ...
        self.spbx_stage_image_range = self.create_dbspinbox(
            value=MAX_STAGE_RANGE, max=MAX_STAGE_RANGE, connect_to_fn=self.update_config
        )
        self.layout_stage_image_range = QHBoxLayout()
        self.layout_stage_image_range.addWidget(
            QLabel("Stage max scan range (\u00b5m):")
        )
        self.layout_stage_image_range.addWidget(self.spbx_stage_image_range)

        self.spbx_excess_start_frames = self.create_spinbox(
            value=0, min=0, max=1000, connect_to_fn=self.update_config
        )
        ...
        self.spbx_coverslip_slope_x = self.create_dbspinbox(
            min=-0.10,
            max=0.10,
            precision=4,
            interval=0.0001,
        ...
        self.spbx_coverslip_slope_y = self.create_dbspinbox(
            min=-0.10,
            max=0.10,
            precision=4,
            interval=0.0001,
        ...
        self.layout_stage_slope.addWidget(self.spbx_coverslip_slope_x)
        self.layout_stage_slope.addWidget(QLabel("y:"))
        self.layout_stage_slope.addWidget(self.spbx_coverslip_slope_y)
        ...
        self.layout_scan_settings.addLayout(self.layout_image_scan_range)
        ...
        self.spbx_crop_x = self.create_spinbox(
            value=self.config["acq_config"]["camera_roi"]["crop_x"],
            max=2000,
            connect_to_fn=self.update_config,
        )
        self.spbx_crop_y = self.create_spinbox(
            value=self.config["acq_config"]["camera_roi"]["crop_y"],
            max=2000,
            connect_to_fn=self.update_config,
        )
        self.spbx_center_x = self.create_spinbox(
            value=self.config["acq_config"]["camera_roi"]["center_x"],
            max=2000,
            connect_to_fn=self.update_config,
        )
        self.spbx_center_y = self.create_spinbox(
        ...
        self.widgets.update(
            {
                "DAQ": {
                    "scan_range_um": self.spbx_image_scan_range,
                    "scan_axis_step_um": self.spbx_scan_step_size,
                },
                "projection_scan": {"camera_readout_us": None},
                "stage_scan": {
                    "max_stage_scan_range_um": self.spbx_stage_image_range,
                    "excess_start_frames": self.spbx_excess_start_frames,
                    "excess_end_frames": self.spbx_excess_end_frames,
                },
                "camera_roi": {
                    "center_x": self.spbx_center_x,
                    "center_y": self.spbx_center_y,
                    "crop_x": self.spbx_crop_x,
                    "crop_y": self.spbx_crop_y,
                },
                "Positions": {
                    "coverslip_slope_x": self.spbx_coverslip_slope_x,
                    "coverslip_slope_y": self.spbx_coverslip_slope_y,
                },
            }
        )
    ...
    # --------------------------------------------------------------------#
    # NOTE: The slide is limited to int, but the spbx allows decimal
    ...
    def update_405_state(self):
        ...
        self.config["acq_config"]["DAQ"]["channel_states"][0] = checked
        self.config["acq_config"]["DAQ"]["channel_powers"][0] = power
        self.config["acq_config"]["DAQ"]["channel_exposures_ms"][0] = exposure_ms
    ...
    def update_488_state(self):
        ...
        self.config["acq_config"]["DAQ"]["channel_states"][1] = checked
        self.config["acq_config"]["DAQ"]["channel_powers"][1] = power
        self.config["acq_config"]["DAQ"]["channel_exposures_ms"][1] = exposure_ms
        self.update_config(update_config=False)
    ...
    def update_561_state(self):
        ...
        self.config["acq_config"]["DAQ"]["channel_states"][2] = checked
        self.config["acq_config"]["DAQ"]["channel_powers"][2] = power
        self.config["acq_config"]["DAQ"]["channel_exposures_ms"][2] = exposure_ms
        self.update_config(update_config=False)
    ...
    def update_638_state(self):
        ...
        self.config["acq_config"]["DAQ"]["channel_states"][3] = checked
        self.config["acq_config"]["DAQ"]["channel_powers"][3] = power
        self.config["acq_config"]["DAQ"]["channel_exposures_ms"][3] = exposure_ms
        self.update_config(update_config=False)
    ...
    def update_705_state(self):
        ...
        self.config["acq_config"]["DAQ"]["channel_states"][4] = checked
        self.config["acq_config"]["DAQ"]["channel_powers"][4] = power
        self.config["acq_config"]["DAQ"]["channel_exposures_ms"][4] = exposure_ms
        self.update_config(update_config=False)
    ...
    def update_config(self, update_config: bool = True):
        ...
        config["acq_config"]["DAQ"]["laser_blanking"] = (
            True if self.cmbx_laser_blanking.currentText() == "on" else False
        )
