...
def assert_standard_image_fields() -> Callable[[Sequence[MDAEvent]], None]:
    ...
    Callable[[Sequence[MDAEvent]], None]
        Assertion checking that stage commands remain on Stage-Move events and
        that camera events retain exposure and physical-position metadata.
    """
    ...
    def _assert(events: Sequence[MDAEvent]) -> None:
        ...
        for event in events:
            ...
            assert event.z_pos is None
            assert set(event.metadata["Stage"]) >= {"x_pos", "y_pos", "z_pos"}
