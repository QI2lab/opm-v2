...
def main() -> None:
    ...
    def custom_execute_mda(output: Path | str | object | None) -> None:
        ...
        #         image_event = False
        #         for ii in trange(len(opm_events), leave=True):
        #             event = opm_events[ii]
        #             if event.index:
        #                 if image_event is False:
        #                     sequence_images = {"action_name": "ImageSequence",
        #                                        "num_images": 0}
        #                     image_event = True
        #                 sequence_images["num_images"] += 1
                        
        #             else:
        #                 if image_event is True:
        #                     events_to_save.append(sequence_images)
        #                     image_event = False
        ...
        #         event_structure_path = output.parent / Path("acq_events.json")
                
        #         with open(event_structure_path, 'w') as file:
        #             json.dump(events_to_save, file, indent=4)
