...
opm_angle_scale = np.sin((np.pi / 180.0) * float(30))
n_active_channels = 2
exposure_ms = 20
# exposure_s = 0.020
pixel_size_um = 0.115
...
if stage_scan_max_range >= range_x_um:
    ...
    n_scan_positions = int(np.ceil(range_x_um / (stage_scan_max_range)))
    scan_tile_length_um = np.round(
        (range_x_um / n_scan_positions)
        + (n_scan_positions - 1) * (scan_tile_overlap_um / (n_scan_positions)),
        2,
    )
scan_axis_step_mm = scan_axis_step_um / 1000.0  # unit: mm
scan_axis_start_mm = min_x_pos / 1000.0  # unit: mm
scan_axis_end_mm = max_x_pos / 1000.0  # unit: mm
scan_tile_length_mm = scan_tile_length_um / 1000.0  # unit: mm
...
scan_axis_positions = np.rint(scan_tile_length_w_overlap_mm / scan_axis_step_mm).astype(
    int
)
...
if DEBUG:
    ...
    print(
        f"Is the scan tile w/ overlap the same as the scan tile length?: {scan_tile_length_mm == scan_tile_length_w_overlap_mm}"
    )
